    ...
    let loadTrackedMvps = pgClient.query('SELECT * FROM guild')
      .then(res => {
        for (let discordGuild of discordClient.guilds) {
          ...
          guildMap.get(discordGuild[0]).talonro = guildDb.talonro;
        }
  ...
  if (msg.author != discordClient.user && msg.channel.guild) {
    ...
    if (guildState) {
      ...
      if (msg.content[0] == "!" && msg.member.hasPermission('ADMINISTRATOR')) {
        ...
        if (argv[0] === "talonro") {
          guildState.talonro = true;
          botReplyMsg = `TalonRO custom MVP respawn times enabled.`;
          pgPool.connect().then(pgClient => {
              insrtOrUpdtSql = 'UPDATE guild SET talonro=true WHERE id=$1'; 
  ...
  discordClient.setInterval(function() {
    for (let discordGuild of discordClient.guilds) {
      ...
      if (voiceChannel && mvpsToNotify.length > 0) {
        voiceChannel.join()
          .then(function(voiceConn) {
            let dispatcher = voiceConn.playFile("audio/pt-br/init.mp3").on('end', reason => {
            ...
            console.error('Error when join voice chat: ' + err);
          });
      }
...
function trackMvpAux(guildState, mvp, minsAgo) {
  ...
  if (guildState.talonro && mvp.t1talonro) {
    mvpState.r1 = mvp.t1talonro - minsAgo;
    mvpState.r2 = mvp.t2talonro - minsAgo;
  }
...
function handleError(err) {
  if (err.code === 'ENOENT' || err.code === 'ETIMEDOUT' || err.code === 'ECONNREFUSED' || err.message === 'read ECONNRESET' || err.message === 'Unexpected server response: 520') {
